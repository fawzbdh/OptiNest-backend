...
class PackingUtil(object):
    @staticmethod
    def getInnerFitRectangle(poly, x, y, offset=(0, 0)):
        left_index, bottom_index, right_index, top_index = GeoFunc.checkBound(poly)
        new_poly = GeoFunc.getSlide(poly, -poly[left_index][0] + offset[0], -poly[bottom_index][1] + offset[1])
            ...
            if kw['fast'] == True:
                ...
                for i in range(1, len(self.polys)):
                    for j in range(0, i):
                        ...
                        self.nfp_list[j][i] = GeoFunc.getSlide(getNFP(self.polys[j], self.polys[i]),
                                                               -self.centroid_list[j][0], -self.centroid_list[j][1])
                # pool.close()
                # pool.join()
                # for i in range(1,len(self.polys)):
                #     for j in range(0,i):
