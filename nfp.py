...
class NFP(object):
    ...
    def main(self):
        ...
        if self.rectangle:  # 若矩形则直接快速运算 点的index为左下角开始逆时针旋转
            width = self.sliding[1][0] - self.sliding[0][0]+ self.offset[0]
            height = self.sliding[3][1] - self.sliding[0][1]+ self.offset[1]
            ...
            while self.judgeEnd() == False and i < 150:  # 大于等于75会自动退出的，一般情况是计算出错
                # while i<7:
                # print("########第",i,"轮##########")
    ...
    def appendWithOffset(self, point):
        # Applies the offset to the point before appending to the NFP list
        offset_point = [point[0] + self.offset[0], point[1] + self.offset[1]]
        self.nfp.append(offset_point)
